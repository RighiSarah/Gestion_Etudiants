<?php
    ...
    class list_groupe extends Master{
        
        public static function ListAll(){
            /*
             
             Méthode pour lister tout les Etudiants
            ...
            /*
             ...
             Méthode pour lister tout les Etudiants pour un groupe choisi
             
             */
            public static function ListByGroup($groupe){
            $Liste = \DataBaseQuery::Get('SELECT utilisateur.* , etudiant.* FROM utilisateur, etudiant 
                                        WHERE utilisateur.IdRole = 5 
                                        AND etudiant.groupe = $1
                                        AND etudiant.NumUtilisateur = utilisateur.NumUtilisateur'
                                        ,$groupe);
            $Liste['nom'] = \DataBaseQuery::Get('SELECT * FROM etudiant WHERE groupe = $1', $Liste['id_etudiant']);
            $Liste['prenom'] = \DataBaseQuery::Get('SELECT * FROM etudiant WHERE groupe = $1', $Liste['id_etudiant']);
            $Liste['mail_etudiant'] = \DataBaseQuery::Get('SELECT * FROM utilisateur, etudiant WHERE etudiant.NumUtilisateur = utilisateur.NumUtilisateur AND etudiant.groupe = $1', $Liste['mail_etudiant']);
            return $Liste;

        }
        
        public static function GetInfo($id_etudiant){
            /*
             
             Méthode pour récuperer toutes les informations relatives à un etudiant
             
             */
            
            
            // Informations de base
            $infos = \DataBaseQuery::Get('SELECT * FROM utilisateur WHERE NumUtilisateur = $1', $id_etudiant);
            if(!empty($infos)){
           ...
           //     $infos['missions'] = \DataBaseQuery::Get('SELECT missions.* FROM missions, missions2etudiants WHERE missions2etudiants.id_mission AND missions2etudiants.id_etu = $1', $id_doctorant);
            //    \App\Model\Master::ArrayFormatter($infos['missions']);
                ...
                // Cursus etudiants
                $infos['cursus'] = \DataBaseQuery::Get('SELECT * FROM `inscrire` INNER JOIN formation ON formation.idFormation = inscrire.idFormation WHERE inscrire.NumEtudiant = $1', $id_etudiant);
        ...
        public static function Update($id,$prenom,$nom,$datenaiss,$rue,$cp,$ville,$pays,$nationalite,$photo){
            /*
             ...
             Méthode de mise à jour des informations de base d'un etudiant
